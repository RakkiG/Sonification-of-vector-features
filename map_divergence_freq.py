...
nu = 1e-6    # Kinematic viscosity, for example, the kinematic viscosity of water is about 1e-6 m^2/s
RHO_WATER = 998.
...
def compute_selected_coefficients(stretched_radiuses_angles, theta_resolution,phi_resolution, selected_indices):
    """
Calculate the stretched radius and theta and phi of each eigenvalue and substitute them into the formula r(θ,φ) = r_0 + c_00 Y_0^0(θ,φ) + c_10 Y_1^0(θ,φ) + c_11 Y_1^1(θ,φ) +
c_20 Y_2^0(θ,φ) + c_21 Y_2^1(θ,φ) + c_22 Y_2^2(θ,φ) coefficients.
    """
    coefficients = []
    ...
    # Iterate over the (l, m) pairs in selected_indices
...
def equation_for_n(n, f_n, sigma, rho, r0):
    """
    solve f_n^2 = (1 / (4 * pi^2)) * (n-1) * (n+1) * (n+2) * (sigma / (rho * r0^3))
...
def linear_interpolate_and_concatenate(signal1, signal2):
    ...
    signal2_no_zeros = signal2_trimmed[:last_nonzero_signal2_idx + 1]
