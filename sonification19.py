# ---For showing the results of vortex coreline
# remove text labels 1
# add texts actors near sources pos
...
for i in range(num_critical_sources, len(sources)):
    random_points = generate_random_points_on_sphere(sources[f"source{i}"].positions[0], 1,50)
...
if vortex == 1:
    for i in range(num_critical_sources, len(sources)):
        ...
        filename_audio = "/Users/gongyaqi/Desktop/MasterThesis/Generate_Sound_For_3D6/vector_field_process5/sounds/wormhole_2.wav"
        ...
        output_filename = f"vortex_signal_{initial_pos_i}.wav"
...
critical_points_actors, lines, vortex_lines_actor, legend_actors = tf.vector_field_actors(filename_vector_field)
...
for i in range(len(sources)):
    for period in inside_periods[f"source{i}"]:
        ...
        fade_in = sigmoid(time_segment, x_shift=0.5, scale=10)

        fade_out = flipped_sigmoid(time_segment, x_shift=0.5, scale=10)
